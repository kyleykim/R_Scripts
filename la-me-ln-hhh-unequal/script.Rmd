...
# Uncomment and run line below if you receive package error message
...
# Uncomment and run these lines if you have not previously installed these packages
...
# This code will spit out your file directory by finding the parent folder. No manually having to set your wd!
here()
```
...
master_data <- read_excel((here("original_data/HHH_list_new.xlsx")))
```

## Clean data
```{r clean, echo=FALSE}
# Set column names
names(master_data) <- c("project_name", "address", "district_no", "units", "sh_units", "status") 

# Remove commas and unit #s for it to geocode properly
master_data$address <- gsub( ",", "", master_data$address) 
master_data$address <- gsub( "#205", "", master_data$address) 
master_data$address <- gsub( "#208", "", master_data$address) 
```
...
```{r barchart, echo=TRUE, message=TRUE, warning=TRUE}
# Install if errored
...
Remove `Eval = FALSE` and put in your key in [API_KEY_HERE] if you want to run the geocoding script. You can also skip this chunk if you want to run the rest of the script.

```{r geocode_script, echo=TRUE, eval = FALSE}
...
```{r mapping_install, warning=F, message=F, quietly=T, echo=T, results='hide'}
...
```{r map, warning=F, message=F, quietly=T, echo=T, results='hide'}
# Join
...
library(svglite) # If you run into svglite issues you may need to download this to resolve https://www.xquartz.org/
library(gdtools)
...
```{r map2, warning=F, message=F, quietly=T, echo=T, results='hide'}
